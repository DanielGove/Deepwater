...
from ..metadata.discovery import list_segments, suggested_reader_range
from .timefmt import (
    add_ts_fmt_arg,
    field_label,
    format_timestamp_us,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        segments = list_segments(args.base_path, args.feed, status=args.status)
    except KeyError:
        print(f"ERROR: feed '{args.feed}' not found", file=sys.stderr)
        return 1

    suggested = suggested_reader_range(args.base_path, args.feed)
