...
with Reader("deepwater-pioneer:/deepwater/data/hyperliquid-node", "trades") as reader:
...
- what feeds exist?
- what metadata settings does a feed have?
...
```python
reader = Reader("dw://deepwater-pioneer:7447/deepwater/data/hyperliquid-node", "trades")
```

Architecture summary:

```text
client Reader("host:/base", "feed")
...
with Reader("deepwater-pioneer:/deepwater/data/hyperliquid-node", "trades") as r:
    print(r.latest(60)[:3])
PY
```
...
def main() -> None:
    ...
    missing = [name for name in required if not feed_exists(base_path, name)]
    if missing:
        names = ", ".join(missing)
        raise RuntimeError(
    ...
    tw = Writer(base_path, "trades")
    tw.write_values(now_us, 100.25, 1.50)
    tw.close()

    qw = Writer(base_path, "quotes")
    qw.write_values(now_us, 100.20, 100.30)
    qw.close()

    tr = Reader(base_path, "trades")
    qr = Reader(base_path, "quotes")
