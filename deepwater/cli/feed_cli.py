#!/usr/bin/env python3
"""
CLI tools for feed metadata operations.
...
from ..metadata.discovery import create_feed, delete_feed, feed_exists
...
def create_main(argv: Optional[list[str]] = None) -> int:
    ...
    for spec in specs:
        name = spec["feed_name"]
        was_present = feed_exists(args.base_path, name)
        try:
            create_feed(args.base_path, spec)
        except Exception as e:
            failed += 1
...
def delete_main(argv: Optional[list[str]] = None) -> int:
    ...
    for name in targets:
        try:
            removed = delete_feed(args.base_path, name, missing_ok=not args.strict_missing)
        except KeyError:
            missing += 1
            failed += 1
