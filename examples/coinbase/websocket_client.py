# examples/coinbase/websocket_client.py
...
class MarketDataEngine:
    ...
    - Direct Deepwater primitives own feed creation and writing.
    ...
    def __init__(self, uri: str = "wss://advanced-trade-ws.coinbase.com",
        ...
        # Deepwater base path
        self.base_path = Path("data/coinbase-test")
    ...
    def _open_writers_for_product(self, product_id: str) -> None:
        ...
        create_feed(self.base_path, trade_spec)
        self.trade_writers[pid] = Writer(self.base_path, trade_spec["feed_name"])

        book_spec = l2_spec(pid)
        create_feed(self.base_path, book_spec)
        self.book_writers[pid] = Writer(self.base_path, book_spec["feed_name"])
    ...
    def _close_writers_for_product(self, product_id: str) -> None:
        """Close product writers."""
        pid = product_id.upper()
        writer = self.trade_writers.pop(pid, None)
        try:
        ...
        writer = self.book_writers.pop(pid, None)
        try:
            if writer is not None:
                writer.close()
        except Exception as e:
    ...
    def _io_loop(self) -> None:
        ...
        bad_frame_dir = self.base_path / "bad_frames"
        try:
